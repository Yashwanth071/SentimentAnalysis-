...
#Building batched dataset objects

BATCH_SIZE = 64
AUTOTUNE = tf.data.AUTOTUNE

def make_dataset(features, labels, batch_size=BATCH_SIZE, shuffle=False):
...
train_ds = make_dataset(X_train_pad, y_train, shuffle=True)
val_ds   = make_dataset(X_val_pad,   y_val,   shuffle=False)
test_ds  = make_dataset(X_test_pad,  y_test,  shuffle=False)
...
def preprocess_single_review(review_text):
...
#examples
examples = [
    "This product was amazing, I loved it and will buy again!",
    "Terrible quality, completely useless and a waste of money.",
    "It was okay. Not great, not terrible, just average."
]

for txt in examples:
    lab, score = predict_sentiment(txt)
    print("Text:", txt)
    print("Prediction:", lab, "| score =", round(score, 4))
    print("-" * 60)
